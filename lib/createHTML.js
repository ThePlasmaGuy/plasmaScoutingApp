...
function createHTML(obj, enforceRequire) {
  console.log(enforceRequire);
  if (enforceRequire === true) {
    catLevel++;
    for (var k in obj) {
    ...
    for (var k in obj) {
      if (typeof obj[k] == "object" && obj[k] !== null) {
        ...
        if (!obj[k].type) {
          ...
          //console.log(obj[k].id);
          if (obj[k].type === "int") {
            html += "<input type='text' id='" + obj[k].id + "' name='" + obj[k].id + "' " + "/><br/>";
          } else if (obj[k].type === "dropdown") {
            html += "<select" + " id='" + obj[k].id + "' name='" + obj[k].id + "' " + ">";
            for (var i = 0; i < obj[k].options.length; i++) {
              html += "<option value='" + obj[k].options[i].value + "'>" + obj[k].options[i].optionName + "</option>";
            }
            html += "</select><br/>";
            ...
            html += "<textarea maxlength='" + obj[k].max + "' minlength='" + obj[k].min + "' id='" + obj[k].id + "' name='" + obj[k].id + "' " + "></textarea><br/>";
          }
        }
      } else {
        if (!obj.type && obj.name) {
