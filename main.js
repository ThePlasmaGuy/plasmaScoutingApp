...
var rawHtml = createHTML.generateHTML(jsonData, true);
...
app.get('/newAnalysis', function(req, res) {
  if (isValidScoutID(req.query.scoutID) === true) {
    var rawHtmlNonRequire = createHTML.generateHTML(JSON.parse(fs.readFileSync("./inputs.json")), false);

    var nonRequireHTML = "<!DOCTYPE html><html><head><title>Scouting App</title></head><body><form id='mainForm' action='/submitAnalysis' method='POST'>" + rawHtmlNonRequire + "<input type='submit' id='submit' value='Submit'/></form><script type='text/javascript'>document.getElementById('mainForm').setAttribute('action', window.location.href)</script></body></html>";
...
app.post('/submitAnalysis', function(req, res) {

  if (debug === true) {
    console.log(req.body);
  }
  res.send(JSON.stringify(searchDB(JSON.parse(fs.readFileSync("./db/db.json")), req.body)));
});
